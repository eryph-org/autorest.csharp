...
namespace AutoRest.Extensions.Haipa
{
    ...
    public abstract class HaipaExtensions : SwaggerExtensions
    {
        ...
        public static void UpdateHeadMethods(CodeModel codeModel)
        {
            ...
            {
                HttpStatusCode successStatusCode = method.Responses.Keys.FirstOrDefault(HttpHeadStatusCodeSuccessFunc);
        ...
        /// <summary>
        /// Set default response to ApiError if not defined explicitly.
        /// </summary>
        ...
        public static void SetDefaultResponses(CodeModel codeModel)
        {
            ...
            // Create ApiError if not already defined
            CompositeType apiError = codeModel.ModelTypes.FirstOrDefault(c =>
                c.Name.EqualsIgnoreCase("ApiError"));
            if (apiError == null)
            {
                apiError = New<CompositeType>( new
                {
                    Name = "ApiError",
                    SerializedName = "ApiError"
                });
                apiError.Extensions[ExternalExtension] = true;
                codeModel.Add(apiError);
            }
            ...
            foreach (var method in codeModel.Methods)
            {
                ...
                {
                    method.DefaultResponse = New<Response>(apiError, method.ReturnType.Headers);
                }                
            }
        }
